...
- **인스펙터 연결 검증 (`Systems/InspectorCheck`)**: 필수 `SerializeField` 누락을 알리는 공용 헬퍼. 각 컴포넌트는 `ValidateReferences()`에서 `InspectorCheck.LogIfMissing`(참조)/`LogIfEmpty`(배열)를 호출하고 결과를 `hasError |= …`로 누적해 **빠진 것을 한 번에 모두** 보고한다(하나 고치면 또 걸리는 식이 되지 않도록). 로그 접두어는 `context.GetType().Name`에서 뽑으므로 클래스명을 문자열로 적지 않는다 — 과거에 복붙으로 다른 클래스 이름이 남아 엉뚱한 파일을 뒤진 적이 있다. 호출 시점은 `Awake` 1회 + `#if UNITY_EDITOR`의 `OnValidate`(플레이 전에 인스펙터에서 발견). 적용: `BattleDirector`·`UnitViewRegistry`·`GameManager`·`CharacterPreview`·`GameUIController`
  ...
  - **누락 시 처리는 두 갈래**: *필수*는 `_isReady` 플래그로 캐싱해 공개 메서드를 막고(`CharacterPreview`, `GameUIController`는 `enabled = false`까지), *연출/선택*은 보고만 하고 사용처가 매번 `!= null`로 건너뛴다(`GameManager._fadeScreenEffect`). **런타임에 파괴될 수 있는 참조는 캐싱하면 안 된다** — Fade Canvas를 GameManager 자식으로 두지 않으면 씬 전환 때 파괴되는데, Awake 시점 스냅샷인 플래그는 그걸 잡지 못한다
...
- **전투 View (`Scripts/Battle/View/`)**: 역할별 컴포넌트로 분리되어 있고 전부 BattleDirector 오브젝트에 붙어 있음
  ...
  - ⚠️ **`UnitViewRegistry`는 `BattleDirector`만 인스펙터로 들고, 나머지 셋(`BattlePresenter`/`MonsterSpawner`/`TargetingController`)은 `Initialize` 주입으로 받는다.** 슬롯을 중복으로 두면 서로 다른 오브젝트를 가리켜도 알아챌 방법이 없다 — Director가 이미 `_registry`를 검증하므로 주입받는 쪽은 null 검사가 필요 없다
