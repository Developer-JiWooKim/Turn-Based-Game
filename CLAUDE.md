...
- **런 데이터 (`Scripts/Progression/Run/`)**: `RunData`가 런 전체 상태를 소유하고 씬을 넘어 `GameManager`가 보관(캐릭터 선택 시 `BeginRun`으로 생성). `RunMember`(파티원 1명 = 원본 SO + 성장 누적 `Stats` + `BaseStats` 스냅샷 + **선택지 몫만 따로 센 `ChoiceGrowth`** + 현재 HP + 런 내내 고정인 `UnitId`) 리스트, `PendingModifiers`(다음 스테이지 몬스터 디버프 예약), `CurrentStage`, `NextUnitId()` 발급기를 가짐. 선택지 효과 적용(`ApplyChoice`)·스테이지 자동 성장(`ApplyStageGrowth`)·전투 결과 반영(`SyncFromBattle`)·사망자 영구 추방(`RemoveFallen`)·영입(`Recruit`)·파티가 꽉 찼을 때 교체(`ReplaceMember`)가 전부 여기 있고 View는 결과만 화면에 반영. `PreviewRecruitStats`는 `Recruit`과 같은 소급 성장 계산(`ApplyCatchUp`)을 공유해, 영입 후보 카드에 뜨는 값과 실제 합류 결과가 항상 일치하도록 함
  - ⚠️ **영입자가 기존 파티원보다 약한 건 버그가 아니다**(확정된 규칙). 다만 **2026-08-12에 "전혀 소급하지 않음" → "부분 소급"으로 바뀌었다.** `ApplyCatchUp`은 **스테이지 자동 성장은 전부**, **선택지 성장은 `StageScaling.RecruitChoiceCatchUpRate`(현재 0.6) 비율만** 소급한다
    - 규칙을 바꾼 이유: 선택지가 비율로 바뀌면서 후반에 고른 한 장이 초반 것의 몇 배 값어치가 되자 격차가 감당 불가가 됐다(36스테이지 영입자가 기존 파티원 ATK의 **25%**). 시뮬레이션에서 그 약한 영입자가 제 몫을 못 해 **다른 파티원까지 연달아 쓰러지는 연쇄**가 확인됐다(`BalanceCurve.md` G절)
    - ⚠️ **`_playerAtkRate` 같은 자동 성장률을 올리는 것은 이 문제의 해법이 아니다** — 영입자와 기존 파티원이 똑같이 받는 데다 선택지 비율의 기준(`scaleBase`)까지 함께 키워서, 3배로 올려도 영입자 비중은 25% → 37%뿐이고 파티 전체 화력만 2.4배가 되어 난이도 곡선이 무너진다
    - 소급분은 **`ChoiceGrowth`에 누적된다**(→ 하단 파티 스탯의 파란 `(+선택지)` 괄호에 잡힌다). 표시 취향이 아니라 **계산상 필수**다 — `CreateScaleBase`가 `Stats − ChoiceGrowth`로 비율 기준을 만들기 때문에, 여기 넣지 않으면 소급분만큼 기준이 부풀어 **그 멤버만 이후 선택지를 더 받고 기존 파티원을 추월한다**
    - ⚠️ **기증자(`CreateCatchUpDonor` = 현재 파티원 `ChoiceGrowth` 평균)를 만드는 시점이 두 곳에서 중요하다.** `Recruit`은 **`AddMember` 전에**(새 멤버의 0이 평균을 끌어내리지 않도록), `ReplaceMember`는 **`Members.Remove` 전에**(교체 대상도 그 성장을 함께 벌어들였으므로. 빼면 "오래 버틴 멤버를 교체할수록 후임이 약해지는" 반대 결과가 난다) 만든다
    - 과거에 "영입 스탯 불일치"로 두 번 의심한 적이 있는데, **그때의 결론(버그 아님)은 지금도 유효하다** — 영입자는 여전히 기존 파티원보다 약하다(60% 소급 기준 ATK의 약 75%). 파란 괄호가 **비어 있는** 것이 정상이었던 서술만 뒤집혔다: 이제는 채워져 있되 베테랑보다 작은 것이 정상이다
  - **선택 횟수 집계**: `ApplyChoice(effect, category)`가 카테고리별 누적 횟수를 `RunData`가 소유한 배열에 센다(HUD의 '선택 기록' 패널용). **스탯 계산에는 전혀 쓰이지 않는 표기 전용 값**이며 **9종을 가리지 않고 전부** 센다. 조회는 `GetChoicePicks()`(0인 항목은 빠진다), 세이브는 `RunSnapshot.ChoicePicks` ↔ `RestoreChoicePicks`
    - ⚠️ **영입은 후보 화면에서 '영입 안 함'으로 물러나도 세어진다** — 집계 시점이 `ApplyChoice`(카드를 고른 순간)이고 영입 후보 제시는 그 뒤이기 때문이다. "영입 ×3인데 합류한 건 2명"이 될 수 있다. 실제 합류만 세려면 `RoguelikeRewardService.PresentRecruitAsync`의 결과를 보고 세야 한다
    - ⚠️ 세이브가 **enum 순서에 묶여 있다** — `RoguelikeCategory`는 뒤에만 추가할 것. 중간에 끼우면 기존 체크포인트의 횟수가 엉뚱한 항목에 붙는다(표기만 틀리고 진행에는 영향 없음)
...
- **파티 시너지 (`Progression/Run/PartySynergyTracker`)**: 같은 캐릭터가 `CharacterStatsSO.SynergyThreshold` 이상 모이면 그 캐릭터들에게만 스탯 보너스(`CreateSynergy()` → `SynergyBonus`, HP는 대상 제외라 회복 부작용 없음).
  ...
  - **`RoguelikeEffect`를 재사용하지 않는다**(과거엔 재사용했다). 2026-08-12에 선택지도 비율로 바뀌어 "고정 증분이라 곱셈 개념이 없다"는 원래 이유는 사라졌지만, **비율의 기준과 되돌리는 방식이 다르다** — 선택지는 `기준값+자동성장`을 기준으로 런 데이터에 영구 누적하고, 시너지는 **얹기 직전의 `Stats`**를 기준으로 전투 중에만 붙였다 떼며 제거가 뺄셈이 아니라 스냅샷 복원이다. 몬스터 디버프·영입 플래그까지 함께 들고 있는 것도 그대로다. 비율 시너지는 덤으로 **스탯 리스케일의 영향을 받지 않는다**(마이그레이션 불필요)
...
- **스탯 스케일 (2026-08 전체 10배 리스케일)**: 정수 스탯(HP/ATK/SPD/DEF)은 10배 규모다(캐릭터 HP 1200~1800, ATK 210~400 — 2026-08-12 4차 조정으로 HP·DEF ×1.5 / ATK ×1.15 상향된 값). 목적은 "숫자가 커 보이게"가 아니라 **반올림 해상도** — ATK가 12이던 시절엔 표현 가능한 최소 증가폭이 +1 = +8%라 성장이 뭉텅뭉텅 뛰었다. 비율값(치명타/치명피해/저항/성장률/스킬 배율/상태이상 크기)은 **스케일 대상이 아니다**
  ...
  - 스케일 대상 에셋: `CharacterStatsSO` 6종·`MonsterStatsSO` 4종의 HP/ATK/SPD/DEF. `StageScalingSO`(전부 비율)·`SkillSO`(배율·상태이상 크기 전부 비율)는 손대지 않는다. **`RoguelikeChoiceSO`도 2026-08-12부터 전부 비율이라 스케일 대상에서 빠졌다**(과거엔 `_hpFlat`/`_atkFlat`/… 고정값이라 함께 10배 해야 했다)
- **전투 Core (`Scripts/Battle/Core/`, 순수 C#·UnityEngine 비의존)**: `Stats`, `Unit`, `SkillProfile`, `DamageCalculator`(비율감소+크리), `TurnOrder`(SPD 정렬), `BattleState`, `IRandom`/`SystemRandom`, 액션/셀렉터(`IActionSelector`, `PlayerActionSelector`=입력 await, `MonsterAiSelector`=노멀 공격/보스 스킬우선), `BattleEvents`, `BattleSimulation`(async 턴 루프, `BattleOutcome` 반환). **연출 동기화**: 이벤트 인자의 `RegisterPlayback(Task)`로 View 애니메이션 완료를 시뮬레이션이 대기
  - `RoguelikeEffect`: 선택지 1종의 순수 효과. 파티 강화 + 몬스터 디버프 3종 + 영입 플래그를 한 struct에 담음(카테고리별 구현체로 쪼개지 않음). `ApplyTo(stats, scaleBase)`는 스탯만 더하고 "채워야 할 HP량"을 반환 — 현재 HP 규칙은 호출자(`Unit`/`RunMember`)가 각자 처리
    - **선택지 성장은 비율이다**(2026-08-12에 고정 증분에서 전환). 무한 타워라 "ATK +120"은 자동 성장만으로도 기준값의 몇 배가 되는 후반에 아무것도 바꾸지 못한다. 정수 스탯(HP/ATK/SPD/DEF)은 **비율**, 치명타·치명피해·저항은 **%p 가산**인 혼합 모델이며 근거는 `SynergyBonus`와 같다(이 셋은 애초에 자동 성장 대상이 아니라 후반 희석 문제도 없다)
    - ⚠️ **비율의 기준은 현재 스탯이 아니라 `scaleBase` = "기준값 + 스테이지 자동 성장"**(`RunMember.CreateScaleBase`가 `Stats − ChoiceGrowth`로 만든다). 현재 스탯을 쓰면 복리로 부풀어 한 스탯에 몰아주는 게 압도적으로 유리해지고, `BaseStats`만 쓰면 기준이 런 내내 고정이라 고정 증분과 다를 게 없어져 전환한 이유가 사라진다. `ChoiceGrowth`가 하단 파티 스탯 표기 말고 **여기서도 쓰인다**
    - ⚠️ **회복(`HealRate`)만 기준이 다르다** — `scaleBase`가 아니라 **실제 MaxHp**다(빈 체력을 채우는 효과라 플레이어가 체력바에서 보는 최대치와 어긋나면 안 된다)
    - ⚠️ **스테이지 자동 성장은 여전히 flat이다.** 같은 struct를 공유하되 생성자가 둘로 갈린다(비율 12인자 = 선택지 / flat 4인자 = `StageScaling.CreatePlayerGrowth`). 자동 성장은 "누적 총량의 차분"이 목적이라 비율로 넘기면 그 정밀도가 사라진다 — 새 성장 경로를 붙일 때 어느 쪽인지 먼저 고를 것
    - ⚠️ **카드 문구는 계산 결과가 아니라 손으로 적은 번역문**이다. 수치를 바꾸면 `UiStringTable.asset`의 `choice.*.desc`(Ko/En)와 에셋의 `_description` 폴백을 함께 고쳐야 한다
  ...
  - `StageScaling`: 스테이지별 양측 스탯 성장. 몬스터는 스폰 시 **배율**(복리 옵션), 플레이어는 HP를 이어받으므로 배율 대신 `BaseStats` 기준 **flat 증가를 영구 누적**. `CreatePlayerGrowth(baseStats, step)`은 매 스테이지 개별 반올림 대신 "누적 총량(`기준값×비율×step`)의 차분"을 돌려줘 반올림 오차가 쌓이지 않음(과거엔 스테이지마다 최소 +1 바닥값을 둬서 저스탯 캐릭터가 비정상적으로 급성장하는 문제가 있었음). 치명타·저항은 스케일링 제외(로그라이크 선택지로만 성장)
    - **난이도 가속**(2026-08-10): `_accelStartStage`(30)까지는 기본 성장률, 그 뒤는 `_accelMultiplier`(2.0)를 곱한 성장률로 전환한다. 두 구간을 이어 붙이므로 경계 이전 누적값이 보존된다. **성장률만 통째로 올리면 초반까지 함께 어려워지므로** 구간을 끊는다
    - ⚠️ **밸런싱 기준선은 4인 파티다**(2026-08-12 확정). 솔로가 아슬아슬한 것은 버그가 아니라 "빨리 사람을 모아라"라는 압력이며, `Recruit.asset`의 `_weightPerEmptySlot`는 난이도 손잡이가 아니라 4인에 빨리 도달시키는 장치다. 난이도는 몬스터 성장률·가속으로 조절한다
    - ⚠️ **HP 성장률과 ATK 성장률은 하는 일이 다르다** — HP는 "전투가 몇 라운드짜리인가", ATK는 "안전율의 높이"를 움직인다. 30 이전 곡선이 우상향하던 것을 평평하게 만든 건 ATK 쪽이고, 후반 벽을 만드는 건 HP 쪽이다(`BalanceCurve.md` F-2)
  ...
  - **전열/후열 = 어그로 가중 추첨**(2026-08-11): 몬스터의 단일 대상 선택이 균등이 아니라 `Unit.AggroWeight`에 비례한다(`MonsterAiSelector.PickTarget` 한 곳). 화면 배치는 그대로이고 **확률만** 다르다
    ...
    - ⚠️ **광역기(`TargetScope.Line`)는 `PickTarget`을 타지 않는다** — 후보 전원이 대상이라 대상 추첨 자체가 없다
    - **도발(Taunt)은 2026-08-19에 범위에서 빠졌다**(`TODO.md`의 '구현하지 않기로 한 것'). 끼어들 자리였던 `PickTarget`은 그대로이고, `ActiveStatus`는 Kind/Magnitude/RemainingTurns 그대로다 — **시전자 Id 필드를 추가할 이유가 없어졌다**
...
- **전투 View (`Scripts/Battle/View/`)**: 역할별 컴포넌트로 분리되어 있고 전부 BattleDirector 오브젝트에 붙어 있음
  ...
  - `BattlePresenter`: Core 이벤트 구독/해제(`Bind`가 먼저 `Unbind`를 호출해 짝이 어긋날 수 없음) → **이벤트를 화면 어디에 반영할지**(HUD 갱신·시너지 표시·행동 유닛 강조·상태이상 표기·도트 팝업)와 시뮬레이션을 대기시키는 `RegisterPlayback`을 맡는다. 레지스트리는 `MonsterSpawner`·`TargetingController`와 같이 `Initialize(ct, registry)` 주입 방식(인스펙터 슬롯 없음)
  - `ActionPlayback`(순수 C#): **유닛 행동 1회의 연출 시퀀스** 전담 — 대상 바라보기 → 공격 애니메이션 → 타격 시점 대기 → 투사체 비행 → 피격·이펙트·숫자 일괄 발동 → 제자리 복귀. 2026-08-19에 `BattlePresenter`(532줄)에서 갈라냈다. "이벤트→화면 반영"과 "한 번의 행동을 어떻게 보여줄 것인가"는 바뀌는 이유가 다르다
    - ⚠️ **MonoBehaviour로 만들지 않는다.** 연출 참조 5종(`_impactDelay`·`_damagePopups`·`_hitEffects`·`_projectiles`·`_cameraShake`)은 `BattlePresenter`의 `[SerializeField]`로 남겨 두고 `Initialize`에서 생성자로 넘긴다 — 컴포넌트로 쪼개면 인스펙터 슬롯이 새 오브젝트로 흩어져 **씬 배선을 다시 해야 한다**. `PartyStatusBarView` 선례와 같은 이유다
    - `RegisterPlayback`은 함께 옮기지 않았다 — 연출 Task를 **이벤트에 등록하는 것**은 구독자(Presenter)의 몫이고, `IsPlayingBack` 카운트도 그쪽이 소유한다
  ...
  - **배틀 패널 4종은 `Battle/View/Panels/`에 모여 있다**(`RoguelikeChoicePanel`/`BattleResultPanel`/`BattlePausePanel`/`MonsterInfoPanel`). 화면 UI 패널과 같은 `BasePanelUI`를 상속하고, 루트 엘리먼트 이름은 인스펙터가 아니라 `RootElementName` 오버라이드로 **코드에 고정**한다(UXML과 1:1이라 인스펙터에 빈칸을 남길 이유가 없다)
    - 네임스페이스도 폴더를 따라 `…Battle.View.Panels`다(`Progression.Run`·`Battle.Data` 선례). **패널이 `Battle.View`의 타입을 쓸 때는 using이 필요 없다** — 자식 네임스페이스에서 부모는 자동으로 탐색된다. 반대 방향(`Battle.View` → 패널)만 `using …Battle.View.Panels;`가 필요하다
  ...
  - 그 외: `UnitView`(`UnitAnimator`/`UnitHealthBar`를 물고 제어), `UnitAnimator`(트리거 재생 + 연출 길이 반환), `UnitHealthBar`(uGUI 월드스페이스 게이지 + TMP 숫자 표기 + 상태이상/스폰 디버프 표기. 사망 시 `PlayDieAsync`가 `SetVisible(false)`로 숨기고 스폰 시 `Initialize`가 되살린다 — 풀에서 재사용되므로 복구를 빼먹으면 그 인스턴스는 영영 체력바가 안 보인다), `TargetingController`, `BattleHUD`(스테이지/턴 순서/현재 행동 유닛/플레이어 차례 프롬프트), `RoguelikeChoicePanel`, `CameraShake`, `DamagePopup`/`DamagePopupSpawner`, `HitEffectSpawner`, `PartyStatusBarView`, `SynergyPanelView`, `ChoicePickPanelView`
    - `ChoicePickPanelView`(화면 이름 **'선택 기록'**): 시너지 표 **바로 아래**에 지금까지 고른 선택지의 카테고리별 누적 횟수를 찍는다(`공격력 강화 ×3`). **9종을 가리지 않고 전부** 센다(성장·회복·몬스터 디버프·영입). 순수 C# View라 `BattleHUD`가 필드로 들고 `choice-pick-panel` 컨테이너를 `Build`로 넘긴다. 한 번도 고르지 않은 항목은 행이 숨겨지고, 전부 0이면 패널째 숨는다(시너지 패널과 같은 규칙)
      - 행 이름은 선택지 SO가 아니라 **카테고리에서 만든다**(`choice.<카테고리>.title` — `RoguelikeChoiceSO.Title`과 같은 키 규칙). 덕분에 HUD가 `RoguelikeChoiceSO` 배열을 들고 있을 필요가 없다. 대신 **아이콘은 표시하지 않는다**(아이콘은 SO에만 있다)
      - 갱신은 `BattleDirector`가 스테이지 시작에 1회(`SetChoicePicks`). 값이 바뀌는 시점이 선택지를 고른 직후뿐이라 전투 중 갱신이 필요 없다 — 시너지처럼 사망 시 다시 밀어 넣지 않는다
      - ⚠️ **UXML의 `left-column`을 없애고 `synergy-panel`에 `top`을 되돌리지 말 것.** 시너지 패널은 행 수에 따라 높이가 변하고 시너지가 없으면 통째로 숨겨져서, 아래 패널에 `top`을 고정하면 겹치거나 허공에 뜬다. 두 패널을 세로 열 하나에 담고 **절대 배치는 그 열에만** 준다
      - **토글 버튼은 시너지와 별도다**(`pick-toggle`, 라벨 `ui.hud.picksToggle`). 두 표가 같은 세로 열에 있지만 하나를 껐다고 다른 하나까지 사라지면 무엇을 껐는지 알 수 없다. 시너지를 끄면 이 패널이 위로 올라온다. 우상단 버튼 순서(시너지 → 성장 → 스탯)는 화면의 패널 순서와 맞춰 뒀다
...
- **전투 Data (`Scripts/Battle/Data/`)**: `UnitStatsSO`(베이스) + `CharacterStatsSO`/`MonsterStatsSO`(임시 스탯, 밸런싱 TBD), `SkillSO`(스킬 1종 = 쿨타임·범위·배율·상태이상. **유닛 종류에 묶이지 않은 별도 에셋**이라 `MonsterStatsSO`가 참조만 한다. 여러 유닛이 같은 스킬 에셋을 공유해도 됨. ⚠️ **캐릭터 스킬이 범위에서 빠졌으므로 이 타입의 사용자는 몬스터뿐이다** — 유닛 종류에 안 묶인 구조는 그대로 두되, 이제 "확장 지점"이 아니라 그냥 현재 설계다), `CharacterRosterSO`(선택 6종), `SpawnWaveSO`(스테이지별 몬스터 구성 + `Weight`/`IsBossWave`— 후자는 `MonsterStatsSO.Tier`로 계산해 별도 플래그와 어긋날 여지 없음), `RoguelikeChoiceSO`(9종 카테고리 + 효과 수치 + 등장 가중치), `StageScalingSO`(양측 성장률 6종 + 복리 토글)
...
### 아직 안 된 것
- **몬스터 정보 팝업(Tab)**: 코드는 완료됐고 **에디터 세팅만 남았다** — BattleScene에 `MonsterInfoPanel`용 UIDocument(Source Asset = `UI/Battle/MonsterInfo.uxml`, Panel Settings = `BattleSettings`)를 만들어 `BattleDirector._monsterInfo` 슬롯에 연결하면 된다. `InputControls.MenuMonsterInfo`(Tab)는 이미 Menu 맵에 있고 리바인딩 대상에도 들어 있다
- ⚠️ **캐릭터 스킬 시스템은 2026-08-19에 범위에서 제외됐다** — 캐릭터 고유 스킬(액티브/패시브)·우하단 스킬 버튼(Z키)·도발 상태이상·스킬 설명 UI를 **전부 하지 않는다**. 캐릭터의 개성은 스탯·전열/후열 어그로·파티 시너지·로그라이크 선택지로만 표현한다. 근거와 착수 전 정리해 뒀던 걸림돌은 `TODO.md`의 '구현하지 않기로 한 것'에 남아 있다
  - **`SkillSO`와 `Unit.IsSkillReady`/`ConsumeSkill`/`TickCooldown`은 그대로 둔다** — 몬스터가 실제로 쓰는 코드다. "플레이어가 안 쓰니 죽은 코드"로 보고 지우지 말 것
  - `Unit._skillCooldownRemaining`을 노출할 이유가 없어졌고(스킬 버튼 전용 요구였다), `ActiveStatus`에 **시전자 Id 필드를 추가할 이유도 없어졌다**(도발 전용 요구였다)
  - `InputControls`·`.inputactions`에 **Z키를 추가하지 않는다**
