...
namespace ToonShadersPro.URP
{
    ...
    {
        // Per-instance override: 볼륨은 High Quality Masked 설정(두께/페이드 등)을 공유하되,
        // 이 인스턴스만 특정 레이어를 특정 색으로 그리도록 색·레이어를 덮어쓴다.
        // (여러 인스턴스를 추가해 레이어별로 다른 아웃라인 색을 낼 때 사용. 기본값이면 볼륨 그대로.)
        [Header("Per-Instance Override (선택)")]
        [SerializeField] private bool _useOverride = false;
        [SerializeField] private Color _overrideColor = Color.white;
        [SerializeField] private LayerMask _overrideLayerMask = 0;
        ...
        public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
        {
            ...
            pass.useOverride = _useOverride;
            pass.overrideColor = _overrideColor;
            pass.overrideLayerMask = _overrideLayerMask;
        ...
        {
            // OutlineFeature가 매 프레임 주입하는 인스턴스별 오버라이드(색/레이어).
            ...
            public RendererListHandle GetRendererList(RenderGraph renderGraph, UniversalRenderingData renderingData, Camera camera, OutlineSettings settings, List<ShaderTagId> shaderTagIDs, Material material)
            {
                ...
                var rendererListDesc = new RendererListDesc(shaderTagIDs.ToArray(), cullingResults, camera)
                {
                    ...
                    layerMask = (useOverride ? overrideLayerMask : settings.objectMask.value).value,
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                {
                    material.SetColor("_OutlineColor", useOverride ? overrideColor : settings.outlineColor.value);
