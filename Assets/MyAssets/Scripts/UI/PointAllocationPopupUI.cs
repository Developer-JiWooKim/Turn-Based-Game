...
{
    /// <summary>
    /// 성향 포인트 배분 팝업(OptionPopupUI와 같은 오버레이 패턴).
    /// 최고 스테이지 돌파로 얻은 영구 포인트를 로그라이크 카테고리별 등장 가중치에 투자한다.
    /// 카테고리 행은 데이터(할당된 선택지 SO) 기반으로 C#에서 동적 생성하고,
    /// 파일 저장은 팝업을 닫을 때(Hide 오버라이드) 1회만 수행한다.
    /// </summary>
    public sealed class PointAllocationPopupUI : BasePanelUI
    {
        [Tooltip("투자 가능한 카테고리. 표시 이름은 Title, 매핑 키는 Category를 재사용한다(보통 9종 전부).")]
        [SerializeField] private RoguelikeChoiceSO[] _categories;
        [Tooltip("몇 스테이지마다 영구 포인트 1점을 획득하는지(밸런싱 값이라 인스펙터에서 조정).")]
        [SerializeField] private int _stagesPerPoint = 10;

        private Label _headerLabel;
        private VisualElement _rowsContainer;
        private readonly List<CategoryRow> _rows = new List<CategoryRow>();

        /// <summary>카테고리 1종의 행 UI 참조(갱신 시 재조회를 피하기 위해 캐싱).</summary>
        ...
        /// <summary>팝업을 열 때마다 최신 획득 포인트를 반영한다(스테이지 갱신 등).</summary>
        ...
        /// <summary>카테고리별 행(이름 + 현재 포인트 + [-]/[+])을 데이터 기반으로 만든다.</summary>
        private void BuildRows()
        {
            if (_rowsContainer == null || _categories == null)
                return;
        ...
        private int Available(SaveData save) =>
            save.GetEarnedPoints(_stagesPerPoint) - save.GetSpentPoints();

        /// <summary>헤더와 모든 행(현재 포인트 + 버튼 활성 상태)을 세이브 값으로 다시 그린다.</summary>
